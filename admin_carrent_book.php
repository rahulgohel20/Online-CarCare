...
<?php
	...
	$result = mysqli_query($connect,"SELECT * FROM rent ORDER BY id ASC LIMIT $start_from, $limit");
	$msg = "";
	if(isset($_GET['delete_id']))
	{
		 
		$id=$_GET['delete_id'];
		$del = "delete from rent where Id='$id';";

		$Result = mysqli_query($connect,$del);
		if($Result)
		{
			echo "<script>location.replace('admin_carrent_book.php');</script>";
...
<div style="display:block">
	...
	<h2 style="color:white;background-color:black;width:500px;padding:20px;margin-left:35%;font-size:50px;text-align:center">Car Rent Booked </h2>
	...
	<?php
		...
		while($rent = mysqli_fetch_row($result))
		{
			
		?><tr id="mytr">
			<td><?php echo $rent[0]; ?></td>
			<td><?php echo $rent[1]; ?></td>
			<td><?php echo $rent[2]; ?></td>
			<td><?php echo $rent[3]; ?></td>
			<td><?php echo $rent[4]; ?></td>
            <td><?php echo $rent[5]; ?></td>
            

			<td><a href="admin_carrent_book.php?delete_id=<?php echo $rent[0]; ?>" onclick="return confirm('Are you sure to delete !'); "  style="text-decoration:none;background:red;padding:5px;
					border:2px solid black;color:white">DELETE</a></td>
			<td><a href="carrent_book_update.php?updateid=<?php echo $rent[0]; ?>"  style="text-decoration:none;background:green;padding:5px;
	...
	$result_db = mysqli_query($connect,"SELECT COUNT(id) FROM rent"); 
